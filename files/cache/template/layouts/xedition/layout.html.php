...
<!--#Meta:layouts/xedition/css/layout.css--><?php $__tmp=array('layouts/xedition/css/layout.css','','','',array());Context::loadFile($__tmp);unset($__tmp); ?>
<!--#Meta:layouts/xedition/css/idangerous.swiper.css--><?php $__tmp=array('layouts/xedition/css/idangerous.swiper.css','','','',array());Context::loadFile($__tmp);unset($__tmp); ?>
<!--#Meta:layouts/xedition/css/welcome.css--><?php $__tmp=array('layouts/xedition/css/welcome.css','','','',array());Context::loadFile($__tmp);unset($__tmp); ?>
<!--#Meta:layouts/xedition/css/webfont.css--><?php $__tmp=array('layouts/xedition/css/webfont.css','','','',array());Context::loadFile($__tmp);unset($__tmp); ?>
<?php if($__Context->mid === 'xeicon'){ ?><!--#Meta:layouts/xedition/css/xeicon.css--><?php $__tmp=array('layouts/xedition/css/xeicon.css','','','',array());Context::loadFile($__tmp);unset($__tmp);
} ?>
<!--#Meta:common/css/xeicon/xeicon.min.css--><?php $__tmp=array('common/css/xeicon/xeicon.min.css','','','',array());Context::loadFile($__tmp);unset($__tmp); ?>
		...
		<?php if($__Context->layout_info->enable_intergration_search === 'Y'){ ?><div class="search_wrap">
			<div class="search_area">
				...
				<form action="<?php echo getUrl() ?>" method="get" class="search" >
					...
					<input type="text" name="is_keyword" value="<?php echo $__Context->is_keyword ?>" required="required" title="<?php echo $lang->cmd_search ?>" placeholder="Search" />
				</form>
				<!-- /SEARCH -->
				<a href="#" class="btn_close" title="<?php echo $lang->cmd_xedition_search_close ?>" onclick="return false"><i class="xi-close"></i><span class="blind"><?php echo $lang->cmd_xedition_search_close ?></span></a>
			</div>
		</div><?php } ?>
...
echo getUrl('');
					...
					<?php }elseif($__Context->layout_info->logo_text){ ?>
						<?php echo $__Context->layout_info->logo_text ?>
					<?php }else{ ?>
...
$__Context->_logo_img = 's_logo.png';
			...
			<div class="side">
				<ul>
					...
					<?php if($__Context->layout_info->enable_intergration_search === 'Y'){ ?><li class="click">
						<a href="#" title="<?php echo $lang->cmd_search ?>"><i class="xi-magnifier"></i><span class="blind"><?php echo $lang->cmd_search ?></span></a>
					</li><?php } ?>
					<!-- admin -->
					<?php if($__Context->logged_info->is_admin == 'Y'){ ?><li>
						<a href="<?php echo getUrl('', 'module', 'admin') ?>" target="_blank" title="<?php echo $lang->cmd_management ?>"><i class="xi-cog"></i><span class="blind"><?php echo $lang->cmd_management ?></span></a>
					</li><?php } ?>
					<!-- login -->
					<li class="hover">
						<?php if($__Context->is_logged){ ?>
							...
							<div class="ly ly_login">
								<ul>
									<?php if($__Context->logged_info->menu_list)foreach($__Context->logged_info->menu_list as $__Context->key=>$__Context->val){ ?><li><a href="<?php echo getUrl('act', $__Context->key, 'member_srl', '', 'page', '') ?>"><?php echo lang($__Context->val) ?></a></li><?php } ?>
									<li><a href="<?php echo getUrl('act', 'dispMemberLogout') ?>"><?php echo $lang->cmd_logout ?></a></li>
								</ul>
						...
						<?php }else{ ?>
							<!-- before_login -->
							<a href="<?php echo getUrl('act', 'dispMemberLoginForm') ?>" id="ly_btn"><i class="xi-user"></i><span class="blind"><?php echo $lang->cmd_login ?>/<?php echo $lang->cmd_signup ?></span></a>
							<div class="ly ly_login">
								<ul>
									<li><a id="ly_login_btn" href="<?php echo getUrl('act', 'dispMemberLoginForm') ?>"><?php echo $lang->cmd_login ?></a></li>
			...
			<?php if($__Context->_enable_unb && $__Context->UNB->list){ ?><div class="custom_area">
				<ul>
					<?php if($__Context->UNB->list)foreach($__Context->UNB->list as $__Context->key=>$__Context->val){ ?><li>
			...
			<?php if($__Context->GNB->list){ ?><nav class="gnb" id="gnb">
				...
				<ul>
					<?php if($__Context->GNB->list)foreach($__Context->GNB->list as $__Context->key1=>$__Context->val1){ ?><li>
...
$__Context->_selected_menu = $__Context->val1;
						...
						<?php if($__Context->val1['list']){ ?><ul class="depth2">
							<?php if($__Context->val1['list'])foreach($__Context->val1['list'] as $__Context->key2=>$__Context->val2){ ?><li<?php if($__Context->val2['list']){ ?> class="more"<?php } ?>>
								...
								<?php if($__Context->val2['list']){ ?><ul class="depth3">
									<?php if($__Context->val2['list'])foreach($__Context->val2['list'] as $__Context->key3=>$__Context->val3){ ?><li>
...
$__Context->_subheader_img = 'sub_banner_xeicon.jpg';
	...
	<div class="body <?php echo $__Context->_body_class ?>">
		...
		<?php if($__Context->_selected_menu['list'] && $__Context->layout_info->layout_type === 'sub' && $__Context->layout_info->sidebar_position !== 'none'){ ?><nav class="lnb">
			<ul>
				<?php if($__Context->_selected_menu['list'])foreach($__Context->_selected_menu['list'] as $__Context->key1=>$__Context->val1){ ?><li>
					<a href="<?php echo $__Context->val1['href'] ?>"<?php if($__Context->val1['open_window'] == 'Y'){ ?> target="_blank"<?php } ?>><?php echo $__Context->val1['link'] ?></a>
...
echo $__Context->layout_info->footer_logo_url;
			...
			<div class="f_info2">
				<div class="site_map">
					<?php if($__Context->FNB->list){ ?><ul>
						<?php $__Context->i = 0 ?>
						<?php if($__Context->FNB->list)foreach($__Context->FNB->list as $__Context->key1=>$__Context->val1){ ?><li<?php if(!($__Context->i % 3) && $__Context->i > 0){ ?> class="clear"<?php } ?>>
							<?php $__Context->i++ ?>
							<a href="<?php echo $__Context->val1['href'] ?>"<?php if($__Context->val1['open_window'] == 'Y'){ ?> target="_blank"<?php } ?>><?php echo $__Context->val1['link'] ?></a>
							<?php if($__Context->val1['list']){ ?><ul>
								<?php if($__Context->val1['list'])foreach($__Context->val1['list'] as $__Context->key2=>$__Context->val2){ ?><li>
									<a href="<?php echo $__Context->val2['href'] ?>"<?php if($__Context->val2['open_window'] == 'Y'){ ?> target="_blank"<?php } ?>><?php echo $__Context->val2['link'] ?></a>
								</li><?php } ?>
							</ul><?php } ?>
						</li><?php } ?>
		...
		<div class="f_cr_area">
			...
			<ul class="mobile-footer-member">
				<?php if(!$__Context->is_logged){ ?>
				<li><a href="<?php echo getUrl('act','dispMemberLoginForm') ?>"><?php echo $lang->cmd_login ?></a></li>
				<li><a href="<?php echo getUrl('act','dispMemberSignUpForm') ?>"><?php echo $lang->cmd_signup ?></a></li>
				<?php }else{ ?>
				<li><a href="<?php echo getUrl('act','dispMemberLogout') ?>"><?php echo $lang->cmd_logout ?></a></li>
				<li><a href="<?php echo getUrl('act','dispMemberInfo') ?>"><?php echo $lang->cmd_view_member_info ?></a></li>
				<?php } ?>
			</ul>
		</div>
	</footer>
...
<!-- TOP -->
<a href="#" class="btn_top"><i class="xi-angle-up"><span class="blind"><?php echo $lang->cmd_move_up ?></span></i></a>
<!-- /TOP -->
<!-- Ncenter login widget -->
<?php if($__Context->layout_info->use_ncenter_widget == 'Y'){ ?>
...
<?php if($__Context->layout_info->use_login_widget != 'N'){ ?><section class="login_widget"<?php if($__Context->XE_VALIDATOR_ID != 'layouts/xedition/layout/1' || !$__Context->XE_VALIDATOR_MESSAGE){ ?> style="display:none"<?php } ?>>
	<!--#Meta:layouts/xedition/css/widget.login.css--><?php $__tmp=array('layouts/xedition/css/widget.login.css','','','',array());Context::loadFile($__tmp);unset($__tmp); ?>
	...
	<div class="signin">
		...
		<div class="login-body">
			<form action="<?php echo getUrl('', 'act', 'procMemberLogin') ?>" method="post" autocomplete="off"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
				<input type="hidden" name="act" value="procMemberLogin" />
				<input type="hidden" name="success_return_url" value="<?php echo getCurrentPageUrl() ?>" />
				<input type="hidden" name="xe_validator_id" value="layouts/xedition/layout/1" />
				<fieldset>
					<legend class="blind"><?php echo $lang->cmd_login ?></legend>
					<div class="control-group">
						<div class="group">
							<input type="<?php if($__Context->member_config->identifier != 'email_address'){ ?>text<?php }else{ ?>email<?php } ?>" name="user_id" id="uemail" required="required" />
							<span class="highlight"></span>
							<span class="bar"></span>
							<label class="info_label" for="uemail"><?php if($__Context->member_config->identifier != 'email_address'){;
echo $lang->user_id;
}else{;
echo $lang->email_address;
} ?></label>
						</div>
						<div class="group">
							...
							<span class="highlight"></span>
							<span class="bar"></span>
							<label class="info_label" for="upw"><?php echo $lang->password ?></label>
						</div>
					</div>
					...
					<div class="control-group">
						<label class="chk_label" for="keepid_opt">
							...
							<span class="checkbox"></span> <?php echo $lang->keep_signed ?>
						</label>
						<div id="warning">
						...
						<button type="submit" class="btn_submit"><?php echo $lang->cmd_login ?></button>
					</div>
				</fieldset>
			</form>
		</div>
