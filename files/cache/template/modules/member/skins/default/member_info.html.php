...
<h1><?php echo $lang->member_info ?></h1>
...
<dl class="dl-horizontal">
	<?php if($__Context->displayDatas)foreach($__Context->displayDatas as $__Context->item){ ?>
	...
	<?php } ?>
	<dt><?php echo $lang->member_group ?></dt>
	<dd><?php echo implode(', ', $__Context->memberInfo['group_list']) ?></dd>
	<dt><?php echo $lang->signup_date ?></dt>
	...
	<?php if($__Context->memberInfo['member_srl'] == $__Context->logged_info->member_srl || $__Context->logged_info->is_admin == 'Y'){ ?>
		<dt><?php echo $lang->last_login ?></dt>
...
<?php if($__Context->memberInfo['member_srl'] == $__Context->logged_info->member_srl){ ?><div class="btnArea btn-group">
	<?php if($__Context->member_config->identifier == 'email_address'){ ?><a href="<?php echo getUrl('act', 'dispMemberModifyEmailAddress') ?>" class="btn"><?php echo $lang->cmd_modify_member_email_address ?></a><?php } ?>
	<a href="<?php echo getUrl('act','dispMemberModifyInfo','member_srl','') ?>" class="btn"><?php echo $lang->cmd_modify_member_info ?></a>
	<a href="<?php echo getUrl('act','dispMemberModifyPassword','member_srl','') ?>" class="btn"><?php echo $lang->cmd_modify_member_password ?></a>
	<a href="<?php echo getUrl('act','dispMemberLeave','member_srl','') ?>" class="btn"><?php echo $lang->cmd_leave ?></a>
</div><?php } ?>
